...
function initRenewalEmail(membershipObj) {
  ...
  for (const renewalThread of label.getThreads()) {
    for (const message of renewalThread.getMessages()) {
      ...
      const [renewedMemeber] = renewedMembersSection.split('$')[0].split('*').map(d => d.trim()).filter(d => d);
  ...
  const actionText = numberOfNewMembers === 0 ? 'there were no new members': `thanked ${numberOfNewMembers} new members`;
  if (numberOfNewMembers > 0 && !hasNewMembers) {
    // const toBeNotifiedStr = toBeNotifiedEmailArr.join(', ');
    // GmailApp.sendEmail(toBeNotifiedStr, 'GSRC Thank You Posts', 'New Thank You\'s are ready for posting!\nPlease check for errors and if all looks correct, select Publish Now from the draft page here:\nhttps://business.facebook.com/latest/posts/draft_posts?asset_id=204391776753');
    GmailApp.sendEmail(DEV_EMAIL, `${isDev ? '[Develop]' : '[Prod]'} GSRC Thank You Posts`, `Script executed, an error may have occured or this was a test`);
  } else {
    GmailApp.sendEmail(DEV_EMAIL, `${isDev ? '[Develop]' : '[Prod]'} GSRC Thank You Posts`, `Script executed, ${actionText}`);
  }
}
...
function getMembershipType(plainMessage) {
  if (plainMessage.includes('Membership Level: Family Membership'))
    return 'Family Membership';
  if (plainMessage.includes('Membership Level: Individual Membership'))
...
function getPostString(membersArr) {
  ...
  return `Thank you ${membersString} for ${isRenewal ? 'renewing ': ''}your GSRC membership!!!\n#belike${hashTagText} #${isRenewal ? 'renew': 'join'}today`;
}
